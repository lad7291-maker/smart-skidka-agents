...
class TestBrowserActions(unittest.TestCase):
    ...
    def test_check_page_render(self):
        ...
        metrics = self.loop.run_until_complete(
            check_page_render('https://example.com', take_screenshot=False)
        )
    ...
    def test_measure_core_vitals(self):
        ...
        vitals = self.loop.run_until_complete(
            measure_core_vitals('https://example.com')
        )
    ...
    def test_screenshot_product(self):
        ...
        path = self.loop.run_until_complete(
            screenshot_product('https://example.com', full_page=True)
        )
    ...
    def test_check_competitor(self):
        ...
        data = self.loop.run_until_complete(
            check_competitor('https://example.com', timeout=8000)
        )
    ...
    def test_batch_check_pages(self):
        ...
        results = self.loop.run_until_complete(
            batch_check_pages(urls, take_screenshots=False, max_concurrent=1)
        )
    ...
    def test_batch_check_competitors(self):
        ...
        urls = ['https://example.com']
        results = self.loop.run_until_complete(
            batch_check_competitors(urls, max_concurrent=1)
        )
        self.assertEqual(len(results), 1)
